...
from ..core.settings import yolo_teacher_model, yolo_teacher_behavior_model, settings, use_half
...
# 新教师行为模型目标类别码定义：teacher_behavior.pt
...
SITTING_SHOULDER_ANGLE_KEY = "SittingShoulderAngle"
DEFAULT_SITTING_SHOULDER_ANGLE = 105.0
ANGLE_EPSILON = 1e-6
...
def is_keypoint_visible(point) -> bool:
    x, y = point
    return x > 0 and y > 0


def lower_body_visible_count(keypoints_xy) -> int:
    visible_count = 0
    for idx in LOWER_BODY_KPTS:
        if is_keypoint_visible(keypoints_xy[idx]):
            visible_count += 1
    return visible_count


def get_teacher_threshold(key: str, default_value: float) -> float:
    threshold_config = getattr(settings, "Teacher_Thresd", {})
    try:
        return float(threshold_config.get(key, default_value))
    except (TypeError, ValueError):
        return default_value


def calculate_nose_shoulder_angle(keypoints_xy):
    """
    计算 0-6 与 0-5 两条线段在鼻子关键点处的夹角。
    ...
    if not (
            is_keypoint_visible(nose) and
            is_keypoint_visible(left_shoulder) and
            is_keypoint_visible(right_shoulder)):
        return None

    vector_left = [
        left_shoulder[0] - nose[0],
        left_shoulder[1] - nose[1]
    ]
    vector_right = [
        right_shoulder[0] - nose[0],
        right_shoulder[1] - nose[1]
    ]
    ...
    cos_angle = (
        vector_left[0] * vector_right[0] +
        vector_left[1] * vector_right[1]
...
def is_sitting(keypoints_xy):
    """判断是否坐着：下半身缺失/少量可见 + 头肩夹角超过配置阈值"""
    # logger.info("[坐姿检测] 开始坐姿检测...")
    if lower_body_visible_count(keypoints_xy) > 1:
        return False

    nose_shoulder_angle = calculate_nose_shoulder_angle(keypoints_xy)
    if nose_shoulder_angle is None:
        return False

    sitting_shoulder_angle = get_teacher_threshold(
        SITTING_SHOULDER_ANGLE_KEY,
        DEFAULT_SITTING_SHOULDER_ANGLE
    )
    return nose_shoulder_angle > sitting_shoulder_angle + ANGLE_EPSILON
...
def build_teacher_result_list(behavior_results: Dict[str, List[ObjectPosition]], object_types: Dict[str, int]):
    result_list = []
    behavior_keys = ['platform_person', 'standing', 'sitting', 'writing']
    behavior_keys.append('teaching' if 'teaching' in object_types else 'calling')
