...
from ai_quality.infrastructure.kafka.consumer import AiQualityKafkaConsumer, _message_attr
...
from ai_quality.infrastructure.worker_registry import (
    WorkerRegistry,
    WorkerRuntimeState,
    build_worker_status,
)
...
class ControlledAiQualityKafkaConsumer:
    def __init__(
            self,
            consumer,
            worker_id: str,
    ...
    def _consume_one(self, handler: Callable[[VisualTaskMessage], None]) -> bool:
        for raw_message in self._iter_messages_once():
            task_message = AiQualityKafkaConsumer._parse_message(raw_message)
            self._heartbeat(
