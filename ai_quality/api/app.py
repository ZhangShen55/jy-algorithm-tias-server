...
from ai_quality.infrastructure.tias.registry import TiasInstanceStatus, TiasRegistry
from ai_quality.infrastructure.worker_control import (
    WorkerControlStateRepository,
    WorkerDesiredState,
)
from ai_quality.infrastructure.worker_registry import WorkerRegistry
...
async def _require_control_key(request: Request, x_ai_quality_key: str | None = Header(default=None)) -> None:
    config: AiQualityConfig = request.app.state.config
    ...
    provided = x_ai_quality_key
    if header_name != "x-ai-quality-key":
