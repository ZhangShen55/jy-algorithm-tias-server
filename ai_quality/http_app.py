...
from ai_quality.api.app import TiasInstanceApi, create_api_app
from ai_quality.config import AiQualityConfig
from ai_quality.infrastructure.tias.registry import InMemoryTiasRegistry, RedisTiasRegistry, TiasRegistry
from ai_quality.infrastructure.worker_control import (
    InMemoryWorkerControlStateRepository,
    RedisWorkerControlStateRepository,
    WorkerDesiredState,
)
from ai_quality.infrastructure.worker_registry import InMemoryWorkerRegistry, RedisWorkerRegistry


logger = logging.getLogger(__name__)


def create_app(registry: TiasRegistry, config: AiQualityConfig | None = None):
    config = config or AiQualityConfig()
...
def create_app_from_config(config: AiQualityConfig):
